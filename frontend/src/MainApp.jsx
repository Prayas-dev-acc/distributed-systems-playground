      ...
      <nav className="shrink-0 flex items-end gap-0.5 px-4 pt-3 pb-0 bg-deep border-b border-subtle">
        {SCENARIOS.map((s) => {
          ...
          return (
            ...
            >
              <span className={`transition-all duration-200 ${isActive ? "opacity-100" : "opacity-60"}`}>{s.icon}</span>
              <span>{s.label}</span>
              {isActive && (
          ...
          <div className="max-w-5xl mx-auto px-6 py-6">
            {active.id !== "home" && (
              <div className="flex items-center gap-3 mb-6 pb-5 border-b border-subtle">
                ...
                <div>
                  <h2 className={`text-base font-semibold tracking-tight ${active.accent}`}>{active.label}</h2>
                  <p className="text-xs text-tertiary mt-0.5">{active.subtitle}</p>
                </div>
              </div>
            )}
            <ScenarioContent id={activeScenario} serverHealth={serverHealth} sockets={sockets} />
          </div>
        </main>
